...
class Sliderm extends Component {
  ...
  render() {
    ...
    return (
      <div
        className="sm:hidden w-4/5 block min-h-[220px] h-fit relative px-3 py-4 bg-gradient-to-br from-zinc-800/90 to-zinc-900/90 backdrop-blur-xl rounded-2xl border border-purple-500/20 shadow-2xl shadow-purple-500/10"
        onMouseEnter={this.stopAutoplay}
        onMouseLeave={this.startAutoplay}
      >
        {/* Main Slide */}
        <div className="min-h-[220px] relative rounded-xl text-center overflow-hidden">
          <img
          ...
          <button
            className="absolute left-2 top-1/2 transform -translate-y-1/2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white text-lg p-2 rounded-full cursor-pointer shadow-lg shadow-purple-500/50 hover:scale-110 transition-all duration-300"
        ...
        <div className=" flex items-center min-h-28 justify-center mt-4">
          {/* Scroll Buttons */}
          <button
            className={`bg-black bg-opacity-70 text-white text-lg p-2 cursor-pointer ${
              thumbnailScrollIndex === 0 ? "opacity-70" : ""
            }`}
          ...
          <div className="flex overflow-hidden mx-2  h-full">
            {this.slideData.map((slide, index) => (
              <img
                ...
                src={slide.src}
                alt={slide.headline}
                className={`w-[70px] h-[70px] object-cover mx-1 cursor-pointer opacity-70 transition-opacity duration-300 ${
          ...
          <button
            className={`bg-black bg-opacity-70 text-white text-xl p-2 cursor-pointer ${
